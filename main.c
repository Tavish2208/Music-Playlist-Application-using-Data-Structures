...
delete:\n");
 ...
 do{
 if(ptr->next==ptr && strcmp(ptr-
>data,temp)==0){
 printf("One file deleted! Playlist 
is Empty Now!\n");
 head=NULL;
 free(ptr);
 return;
...
void last_node(){
 ...
 printf("Playing Last Music : 
%s\n",head->prev->data);
 }
}
void specific_data(){
 if(head==NULL){
 printf("No music is there to be 
searched!\n");
7
 return;
 }
...
>data);
 ...
 printf("There is no Music file with this 
name!\n");
}
int main(){
 int choice;
